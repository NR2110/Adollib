...
namespace Adollib
{
	...
	private:
		...
		Collider_Rope* Lrope_coll = nullptr; //ropeのcollider
		Joint_base* Lblock_rope_joint = nullptr; //ropeとblockのjoint
		Joint_base* Lblock_hand_joint = nullptr; //腕とblockのjoint(ropeの長さが0になった場合ここを直接つなげる)
		int nearest_rope_collider_num = 1; //ropecollidersの内一番手に近いcolliderのID
		float dither_timer = 0; //予測線を出す際のdithertimer
		float rope_sphere_r = 0.2f; //ropeの半径
		Gameobject* rope_hit_sphere = nullptr; //予測線の先端のsphere
		Gameobject* rope_hit_cylinder = nullptr; //予測線の線
