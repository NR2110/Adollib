...
void Player::catch_things() {
	...
	// stageÇ™ïœçXÇ≥ÇÍÇΩÇÁgameobjectÇÃçÌèúÇ≈jointÇÕè¡Ç¶ÇƒÇ¢ÇÈÇÃÇ≈ jointÇçÌèú
	...
	for (int i = 0; i < 2; i++) {
		...
		Joint_base*& joint = *joints[i];
		//éùÇ¬
		if (input->getMouseState(key) && joint == nullptr && *is_maked_joint[i] == false) { //KeyÇ™âüÇ≥ÇÍÇƒÇ¢ÇÈ jointÇ™ë∂ç›ÇµÇ»Ç¢ ìØstateíÜÇ…ï®ÇÇ¬Ç©ÇÒÇ≈Ç¢Ç»Ç¢
...
void Player::add_pow_for_stand() {
	...
	is_gunyatto = false;
	//óºéËÇ™staticÇ»Ç‡ÇÃÇéùÇ¡ÇƒÇ¢ÇÈÇ∆Ç´ÅAÇÆÇ…Ç·Ç¡Ç∆
...
void Player::push_waist_for_stand() {
	...
	if (dot < 0)return; //çòÇ™â∫Çå¸Ç¢ÇƒÇ¢Ç»ÇØÇÍÇŒreturn
	dot = dot * dot;
	...
	//		float mass = 10 * 4; //ÉpÅ[ÉcÇ…ÇÊÇ¡Çƒéøó Ç™Ç‹ÇøÇ‹ÇøÇ»ÇÃÇ≈ ÇæÇ¢ÇΩÇ¢ÇÃéøó 
	//		Vector3 gravity_pow = Vector3(0, 1, 0) * Physics_function::Physics_manager::physicsParams.gravity * mass; //playerÇ™ëÿãÛèoóàÇÈóÕ
	//		Vector3 force = Vector3(0, 1, 0) * (stand_dis * dot - dis) * stand_pow; //ÇﬂÇËçûÇ›Çé°Ç∑óÕ
	...
	if (onground_collider != nullptr) {
		constexpr float mass = 10 * 4; //ÉpÅ[ÉcÇ…ÇÊÇ¡Çƒéøó Ç™Ç‹ÇøÇ‹ÇøÇ»ÇÃÇ≈ ÇæÇ¢ÇΩÇ¢ÇÃéøó 
		...
		Vector3 gravity_pow = Vector3(0, 1, 0) * Physics_function::Physics_manager::physicsParams.gravity * mass; //playerÇ™ëÿãÛèoóàÇÈóÕ
		Vector3 force = Vector3(0, 1, 0) * (onground_dis * dot - dis) * stand_pow; //ÇﬂÇËçûÇ›Çé°Ç∑óÕ
		Vector3 fall_force = Vector3(0, 1, 0) * Waist_collider->linear_velocity().y * mass;
...
void Player::linear_move() {
	...
	// ì¸óÕÇ™Ç†ÇÍÇŒìÆÇ´Ç‚Ç∑Ç¢ÇÊÇ§Ç…0.1Ç…Ç∑ÇÈ
	else {
	...
	if (!input->getKeyState(Key::LeftControl))
	{
		...
		if (onground_collider != nullptr) {
			if (onground_collider->physics_data.inertial_mass > 20) //ï‡Ç≠Ç∆Ç´Ç…è¨Ç≥Ç»êŒÇ±ÇÎÇ…ëSóÕÇâ¡Ç¶Ç»Ç¢
...
void Player::move_legs() {
	...
	if (dir.norm() != 0) {
		const float front_value = ToRadian(30); //ë´ÇÕì∑ëÃÇÊÇËíxÇÍÇÈÇΩÇﬂ ë´ÇëOÇ…èoÇ∑ÇΩÇﬂÇ…ó]ï™Ç…Ç©ÇØÇÈâÒì]ÇÃóÕ
		const float max_theata = ToRadian(60) * 0.5f;
		const float sin = sinf(move_timer * 1.5f * DirectX::XM_2PI); //timreÇ©ÇÁë´ÇÃâÒì]ó ÇãÅÇﬂÇÈ
...
void Player::make_jump() {
	...
	if (onground_collider != nullptr && //ê⁄ínÇµÇƒÇ¢ÇÈ
		(catch_right_joint == nullptr || catch_right_joint->get_colliderB() != onground_collider) && //âEéËÇ≈éùÇ¡ÇƒÇ¢ÇÈÇ‡ÇÃÇ≈ÇÕÇ»Ç¢
		(catch_left_joint  == nullptr || catch_left_joint->get_colliderB()  != onground_collider)    //ç∂éËÇ≈éùÇ¡ÇƒÇ¢ÇÈÇ‡ÇÃÇ≈ÇÕÇ»Ç¢
		)is_jumpable = true;
...
bool Player::check_respown() {
	...
	// stageéwíËÇÃYç¿ïWÇÊÇËPlayerÇ™í·ÇØÇÍÇŒrespown
...
void Player::turn_gunyatto_dir() {
	...
	// çòÇ©ÇÁì™Ç÷ÇÃå¸Ç´(Ç±ÇÃï˚å¸Ç…ê≥ñ ÇéùÇ¡ÇƒÇ¢Ç´ÇΩÇ¢)
	...
	// óßÇ¡ÇƒÇ¢ÇÈÇ»ÇÁâΩÇ‡ÇπÇ∏Ç…return
	else return;
	...
	// âÒì]é≤ÇÕ0,1,0Ç…ÇµÇΩÇ¢Ç©ÇÁ äOêœÇ≈âÒì]ï˚å¸ÇÃämîF
