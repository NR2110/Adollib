...
void Player::reach_out_hands() {
	...
	/*
	{
		...
		//addforce‚ÌŒü‚«‚ğ•K—v‚É‰‚¶‚Ä‹t‚É‚µ‚È‚¯‚ê‚Î‚¢‚¯‚È‚¢ —vC³
...
void Player::catch_things() {
	...
	for (int i = 0; i < 2; i++) {
		...
		// static‚È‚à‚Ì‚É‹ß‚¢‡(hand‚©‚ç‡)‚É¿—Ê‚ğ‘å‚«‚­‚·‚é
		if (joint != nullptr && joint->get_colliderB()->physics_data.is_moveable == false) {
			// static‚È‚à‚Ì‚É‹ß‚¢‡(hand‚©‚ç‡)‚É¿—Ê‚ğ‘å‚«‚­‚·‚é
...
void Player::tuning_waist_pillar() {
	...
	float center_y = waist_pillar->center.y;
	{
		...
		// ‚µ‚Î‚ç‚­’n–Ê‚É‚¢‚È‚¯‚ê‚Î™X‚Éwaist_Sphere‚ğã‚É‚ ‚°‚é
			anchor_move_vec = -0.2f;
		}
		else anchor_move_vec = 4; // Ú’n‚µ‚Ä‚¢‚ê‚Îwaist_Sphere‚ğ‰º‚°‚é
...

		{
			...
			// timer‚ª0‚æ‚è‘å‚«‚¢ waist_Sphere‚ğã‚É‚ ‚°‚é
