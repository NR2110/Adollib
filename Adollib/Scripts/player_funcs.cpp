...
void Player::push_waist_for_stand() {
	...
	if (onground_collider != nullptr) {
		constexpr float mass = 10 * 4; //ƒp[ƒc‚É‚æ‚Á‚Ä¿—Ê‚ª‚Ü‚¿‚Ü‚¿‚È‚Ì‚Å ‚¾‚¢‚½‚¢‚Ì¿—Ê
		...
		Vector3 force = Vector3(0, 1, 0) * (onground_dis * dot - dis) * stand_pow; //‚ß‚è‚İ‚ğ¡‚·—Í
...
void Player::linear_move() {
	...
	if (!input->getKeyState(Key::LeftControl))
	{
		...
		if (onground_collider != nullptr) {
			if (onground_collider->physics_data.inertial_mass > 20) //•à‚­‚Æ‚«‚É¬‚³‚ÈÎ‚±‚ë‚É‘S—Í‚ğ‰Á‚¦‚È‚¢
