...
void SE_manager::hand_pop_SE() {
	...
	if(time < 0) MonoAudio::PlayMusic(Music::SE_Pop + player_num * 10, false);
	time = 0.5f;
}
void SE_manager::jump_SE() {
	float& time = jump_SE_timer;
	if (time < 0) MonoAudio::PlayMusic(Music::SE_Jump + player_num * 10, false);
	time = 0.5f;
}
void SE_manager::landing_SE() {
	float& time = landing_SE_timer;
	if (time < 0) MonoAudio::PlayMusic(Music::SE_Landing + player_num * 10, false);
	time = 0.5f;
}
void SE_manager::step_SE() {
	float& time = step_SE_timer;
	if (time < 0) {
		if(step_sign == -1) MonoAudio::PlayMusic(Music::SE_Step0+ player_num * 10, false);
		if(step_sign == +1) MonoAudio::PlayMusic(Music::SE_Step1+ player_num * 10, false);
	}
	if (time < 0) {
		time = 0.3f;
		step_sign *= -1;
