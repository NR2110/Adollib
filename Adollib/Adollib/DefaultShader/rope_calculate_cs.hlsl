...
{
    // 頂点の番号
    const uint vertex_num = floor((float) input.dispatch.x / 3);

    const uint index_num = indexces[input.dispatch.x]; //index

    // color[0].wはそのmeshのinstanceのstart位置
    BufOut[instance_num].position[vertex_num] = vertices[index_num].position + vertices_offset[index_num].position;
    BufOut[instance_num].normal[vertex_num] = vertices_offset[index_num].normal;
    //BufOut[color[0].w + instance_num].normal[vertex_mum] = normalize(vertices[index_num].normal + vertices_offset[index_num].normal);
    BufOut[instance_num].color = float4(color[0].xyz, 1);
