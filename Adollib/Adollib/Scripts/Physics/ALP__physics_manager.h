...
namespace Adollib
{
	...
	public:
		...
		static Physics_function::ColliderPhysics_itrs add_collider(Collider* coll, Scenelist Sce = Scene::now_scene) {
			...
			(*ret.coll_itr)->physics_data.drag = Phyisics_manager::default_physics.drag; //空気抵抗
			...
			(*ret.coll_itr)->physics_data.is_kinematic = Phyisics_manager::default_physics.is_kinematic;//影響うけない(fallはする)
