...
bool Calc_joint_effect(ALP_Joint* joint)
{
	...
	if (joint->joint->limit_effect(joint->limit_constraint_pos[0], joint->limit_constraint_pos[1], penetrate)) {
		...
		joint->constraint_limit.rhs += joint->limit_bias * penetrate / Phyisics_manager::physicsParams.time_per_flame; // position error
	...
	// 拘束のセットアップ
	{
		...
		for (auto& joint : joints) {
			...
			for (int i = 0; i < joint->anchor_count; i++) {
				...
				constraint.rhs += joint->bias * (DirectX::XMVectorGetX(distance) - joint->slop) / Phyisics_manager::physicsParams.time_per_flame; // position error
		...
		for (int C_num = 0; C_num < pair->contacts.contact_num; C_num++) {
			...
			// Normal
			{
				...
				if (0.0f < cp.distance - Phyisics_manager::physicsParams.slop)
					cp.constraint[0].rhs += (Phyisics_manager::physicsParams.bias * (cp.distance - Phyisics_manager::physicsParams.slop)) / Phyisics_manager::physicsParams.time_per_flame; //めり込みを直す力
