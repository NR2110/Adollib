...
#pragma region Load FBX
		...
		for (size_t mesh_num = 0; mesh_num < fetched_meshes.size(); mesh_num++)
		{
			...
			// materialが無かったらsubsetのsizeが0になっている それだと死ぬので取りあえず1にresize
					...
					if (is_marge_vertex)
						for (int same_vertex_num = 0; same_vertex_num < vertex_size; ++same_vertex_num) {
							...
							if (vector3_distance(vertex.position, vertices[same_vertex_num].position) < FLT_EPSILON) {
								indices.at(index_offset + index_of_vertex) = static_cast<u_int>(same_vertex_num); //indexに保存して
								vertices[same_vertex_num].normal += vertex.normal; //normalを足して
