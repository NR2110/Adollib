...
namespace Adollib {
	namespace Compute_S {
		...
		public:
			...
			// StructureBufferの作成
			HRESULT create_StructureBuffer(UINT elementSize, UINT count, void* InitData, Microsoft::WRL::ComPtr<StructureBuffer>& ppBufferOut, bool is_upu_access);

			// StructureBufferからSRVの作成
			HRESULT createSRV_fromSB(Microsoft::WRL::ComPtr<StructureBuffer>& pBuffer, Microsoft::WRL::ComPtr<ID3D11ShaderResourceView>& ppSRVOut);

			// StructureBufferからUAVの作成
			HRESULT createUAV_fromSB(Microsoft::WRL::ComPtr<StructureBuffer>& pBuffer, Microsoft::WRL::ComPtr<ID3D11UnorderedAccessView>& ppUAVOut);
			...
			T* Read_UAV(Microsoft::WRL::ComPtr<UAVBuffer>& pBuffer) {
				...
				T* start = static_cast<T*>(subRes.pData);
				auto a = start[0];
				auto b = start[1];
				auto c = start[2];
				auto d = start[740];
