...
void Collider_renderer::awake() {
	//::: ÉRÉìÉXÉ^ÉìÉgÉoÉbÉtÉ@ :::
	Systems::CreateConstantBuffer(world_cb.ReleaseAndGetAddressOf(), sizeof(ConstantBufferPerGO));    //ògämï€
	Systems::CreateConstantBuffer(Mat_cb.ReleaseAndGetAddressOf(), sizeof(ConstantBufferPerMaterial));//ògämï€
...
void Collider_renderer::render_joint(const Physics_function::ALP_Joint* joint) {
	...
	for (int i = 0; i < joint->anchor_count; i++) {
		...
		if (joint->shrink_bias != 0 || joint->stretch_bias != 0)color = Vector4(-1, -1, -1, -1);
