...
bool Physics_function::generate_contact_capsule_mesh(const Collider_shape* capsule, const Collider_shape* mesh, Contacts::Contact_pair*& pair, bool& is_crossing) {
	...
	if (0 && mesh->get_mesh_data()->is_Convex == true) {
		...
		const XMVECTOR capsule_dir_meshcoord = XMVectorScale(XMVector3Rotate(XMVectorSet(0, 1, 0, 0), XMQuaternionMultiply(capusle_Worient, XMQuaternionInverse(mesh_Worient))), capsule->world_scale().y); //mesh座標系でのsphereのpos

		bool is_hit = false; //衝突していたらtrue
		float min_dis = capsule->world_scale().x;//最低距離をsphereの半径に
		float min_t; //衝突点のt
		XMVECTOR closest_point; //mesh上の最近点
		XMVECTOR nor_meshcoord; //衝突点での法線
		XMVECTOR min_sphere_pos_meshcoord; //衝突点のcapsuleのsphereのpos
		//capsuleとmeshの判定
		for (const auto& faset : mesh->get_mesh_data()->facets) {
		...
		const XMVECTOR Wn = XMVector3Normalize(XMVector3Rotate((min_sphere_pos_meshcoord - closest_point), mesh_Worient));//meshからsphereへのベクトル
